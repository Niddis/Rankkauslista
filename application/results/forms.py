...
from wtforms import IntegerField, validators, SelectField, ValidationError, HiddenField, RadioField
...
class ResultEditRankForm(FlaskForm):
    cup_id = HiddenField()
    rank = IntegerField("Sijoitus", [validators.NumberRange(min=0, max=50, message="sijoituksen pitää olla vähintään 0 ja korkeintaan 50")])
...
class ResultEditPointsForm(FlaskForm):
    points = IntegerField("Pisteet", [validators.NumberRange(min=0, max=500, message="pisteiden pitää olla vähintään 0 ja korkeintaan 500")])
